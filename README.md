...
Текущий этап проекта: витрина React с каталогом из PostgreSQL, сохраняемой гостевой корзиной, авторизацией по одноразовому коду, оформлением и историей заказов покупателя, media-сервисом для товарных изображений в MinIO и отдельной админкой. Администратор управляет товарами, размерами, остатками и статусами заказов.
...
- `GET http://localhost:8103/api/v1/orders` — список заказов текущего покупателя; требует `Authorization`.
- `GET http://localhost:8103/api/v1/orders/{order_id}` — состав и история статусов собственного заказа; чужой заказ возвращает `404`.
...
1. API серверной корзины и перенос гостевой корзины после авторизации.
2. Резервирование остатков при подтверждении заказа.
3. Интеграция оплаты в рублях (например, ЮKassa — после предоставления реквизитов).
4. Доставка, уведомления о статусах и операционная наблюдаемость.

Сейчас оформление создаёт реальный заказ со статусом «Новый». Оплата не списывается, доставка согласуется менеджером, а остаток ещё не резервируется автоматически.
